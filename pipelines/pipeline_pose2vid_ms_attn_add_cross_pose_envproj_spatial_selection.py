...
from models.mutual_self_attention import ReferenceAttentionControlv2
...
class Pose2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        reference_control_writer = AttnControl(
            self.reference_unet,
        ...
        reference_control_reader = AttnControl(
            self.denoising_unet,
            do_classifier_free_guidance=do_classifier_free_guidance,
            mode="read",
            batch_size=batch_size,
        ...
        latents = self.prepare_latents(
            ...
            height,
            video_length,
            image_prompt_embeds_list[0].dtype,
            device,
            generator,
        )
        ...
        pose_cond_tensor = torch.cat(pose_cond_tensor_list, dim=2).to(
        ...
        env_code = self.env_proj(env_code.to(device=device, dtype=self.pose_guider.dtype)).unsqueeze(1)
        env_code = torch.cat([env_code] * 2) if do_classifier_free_guidance else env_code
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    attn_fea = [self.attn_guider(pose_cond_input, pose_ref) for pose_ref in ref_pose_cond_tensor_list]
                    
                    attn_fea = [torch.cat([fea] * 2) for fea in attn_fea]
                    pose_fea = [self.pose_guider(pose_cond_input, pose_ref) for pose_ref in ref_pose_cond_tensor_list]
                    pose_fea_fuse = []
                    ...
                    class WrappedModel(torch.nn.Module):
                        def __init__(self, model, timestep, encoder_hidden_states, attn_feat, pose_feat):
                        ...
                        def forward(self, x):
                            return self.model(
                                ...
                                attn_feat=self.attn_feat,
                                pose_feat=self.pose_feat,
                                return_dict=False,
                            )[0]
                    wrapped_model = WrappedModel(self.denoising_unet, t, torch.cat(image_prompt_embeds_list+[env_code],-2), attn_fea, pose_fea)
                    # 使用 ptflops 计算 FLOPs
                    input_size = (1, *latent_model_input.shape[1:])  # 假设批量大小为 1
                    macs, params = get_model_complexity_info(
                        wrapped_model,
                        input_res=input_size[1:],  # 不包括批量维度
                        as_strings=True,
                    ...
                    pred = self.denoising_unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=torch.cat(image_prompt_embeds_list+[env_code],-2),
                        attn_feat=attn_fea,
                        pose_feat=pose_fea,
                        return_dict=False,
                    )[0]
