...
def image_level_evaluation(input_list, verbose=False, output_dir="./debug/"):
    ...
    for image_path in tqdm(input_list):
        ...
        h = W // 4
        gt = [
            image[0*w:1*w, 3*h:4*h, :],
            image[1*w:2*w, 3*h:4*h, :],
            image[2*w:3*w, 3*h:4*h, :]
        ]
        
        res = [
            image[0*w:1*w, 2*h:3*h, :],
            image[1*w:2*w, 2*h:3*h, :],
            image[2*w:3*w, 2*h:3*h, :]
        ]
        gt_list.extend(gt)
        res_list.extend(res)
