...
def process_video(save_dir,fp, video_name):
    pose_pos = np.load(fp+'/' + video_name + "_pose_pos.npy")
    pose_id = np.load(fp+'/'  + video_name + "_pose_id.npy")
    face = np.load(fp+'/'  + video_name + "_face_pos.npy")
    hands = np.load(fp+'/'  + video_name + "_hands_pos.npy")
    whole_score = np.load(fp+'/'  + video_name + "_pose_score.npy")
    body_score = whole_score[:, :18]
    face_score = whole_score[:, 24:92].reshape(-1, 1, 68)
    hands_score = whole_score[:, 92:].reshape(-1, 2, 21)
    
    video_reader = VideoReader(fp.replace('npy','videos')+'/' +video_name+".mp4")
...
def draw_final_ted_dataset():
    ...
    fp = '/mnt/localssd/final_ted_dataset/npy'
    video_files = glob.glob(fp+'/*')
    ...
    for vid in allocated_fps:
        pbar.update(1)
        save_dir = vid.replace('npy','dwpose')
        os.makedirs(save_dir,exist_ok=True)
        # 初始化一个集合，用于存储不重复的id
        unique_ids = set()

        # 遍历目录下的所有文件
        for file_name in os.listdir(vid):
            # 忽略非 .npy 文件
            if not file_name.endswith(".npy"):
                continue
            id_part = "_".join(file_name.split("_")[:4])
            # 添加到集合中
            unique_ids.add(id_part)
        unique_ids = sorted(list(unique_ids))
        # print(unique_ids)
        for id in tqdm(unique_ids):
            if os.path.exists(save_dir+'/' + id + "_weighted_heatmap.mp4"):
                continue
            process_video(save_dir,vid,id)
            
if __name__ == "__main__":
    draw_final_ted_dataset()
def process_mvhuman():
    import argparse
    ...
    # parser = argparse.ArgumentParser()
    # parser.add_argument("--video_dir", type=str, default="/mnt/localssd/champ_trainning_sample/rawdata/")
    # parser.add_argument("--resume", default=False, action="store_true")
    # parser.add_argument(
    #     "--save_dir", type=str, default="/mnt/localssd/champ_trainning_sample/rawdata/_dwpose3/"
    # )
    # args = parser.parse_args()
    ...
    # for video_name in tqdm(todo_video):
    #     process_video(args.save_dir, video_name)  
    # video_files = sorted(glob.glob('/projects/p32321/fating/dataset/CelebV-HQ/35666/*/*.mp4'))
    
    video_files = sorted(glob.glob(args.input_dir+'/*/*/images_lr/*/*.png'))
