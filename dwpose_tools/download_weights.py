...
def prepare_base_model():
    ...
    local_dir = "/mnt/localssd/pretrained_weights/stable-diffusion-v1-5"
...
def prepare_image_encoder():
    ...
    local_dir = "/mnt/localssd/pretrained_weights"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in ["image_encoder/config.json", "image_encoder/pytorch_model.bin"]:
        ...
        if os.path.exists(saved_path):
            continue
        # print(path.parent, path.name, local_dir)
        hf_hub_download(
...
def prepare_dwpose():
    ...
    local_dir = "/mnt/localssd/pretrained_weights/DWPose"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in [
...
def prepare_vae():
    ...
    local_dir = "/mnt/localssd/pretrained_weights/sd-vae-ft-mse"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in [
        "config.json",
...
def prepare_controlnet():
    print(f"Preparing controlnet weights...")
    local_dir = "/mnt/localssd/pretrained_weights/control_v11p_sd15_openpose"
    os.makedirs(local_dir, exist_ok=True)
    for hub_file in [
        "config.json",
        "diffusion_pytorch_model.bin",
