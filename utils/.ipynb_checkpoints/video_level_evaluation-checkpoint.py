...
model_path='/sensei-fs/users/haiyangl/pose2video/utils/models/rgb_charades.pt'
...
def video_level_evaluation(input_list, verbose=False, output_dir="./debug/"):
    ...
    for video_path in tqdm(input_list):
        ...
        for index in range(video_length):
            ...
            image = Image.fromarray(img.asnumpy()).resize((512*4, 512))
            ...
            gt = image[:,3*512:4*512,:]
            res = image[:,2*512:3*512, :]
        ...
        if verbose:
            for i, (gt_patch, res_patch) in enumerate(zip(gt_list, res_list)):
                gt_path = os.path.join(output_dir, f"gt_{os.path.basename(video_path).split('.')[0]}_{i}.png")
                res_path = os.path.join(output_dir, f"res_{os.path.basename(video_path).split('.')[0]}_{i}.png")
