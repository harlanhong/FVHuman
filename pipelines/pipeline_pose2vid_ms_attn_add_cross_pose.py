            ...
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    for i in range(len(attn_fea[0])):
                        if do_classifier_free_guidance:
                            attn_fea_fuse.append([torch.cat([fea[i]] * 2) for fea in attn_fea])
                        else:
                            attn_fea_fuse.append([fea[i] for fea in attn_fea])
                    attn_fea = attn_fea_fuse
                    ...
                    wrapped_model = WrappedModel(self.denoising_unet, t, torch.cat(image_prompt_embeds_list,-2), attn_fea, pose_fea)
                    # 使用 ptflops 计算 FLOPs
                    ...
                    pred = self.denoising_unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=torch.cat(image_prompt_embeds_list,-2),
                        attn_feat=attn_fea,
                        pose_feat=pose_fea,
                        return_dict=False,
                    )[0]
