...
def process_video(save_dir, video_name):
    loaded_data = np.load(f'{save_dir}/data.npz')
    pose_pos = loaded_data['pose_pos']
    pose_id = loaded_data['pose_id']
    face = loaded_data['face_pos']
    hands = loaded_data['hands_pos']
    whole_score = loaded_data['pose_score']
  
    body_score = whole_score[:, :18]
    face_score = whole_score[:, 24:92].reshape(-1, 1, 68)
    hands_score = whole_score[:, 92:].reshape(-1, 2, 21)
    
    video_reader = VideoReader(f'{save_dir}/dwpose.mp4')
    ...
    save_videos_from_pil(hands_map,f'{save_dir}/hands_face.mp4', fps=fps)
    save_videos_from_pil(oris, f'{save_dir}/weighted_heatmap.mp4', fps=fps)
...
def process_single_video(
    ...
    data = {"pose_pos":pose_pos,'pose_id':pose_id,'pose_score':pose_score,'hands_pos':hands_pos,'face_pos':face_pos}
    np.savez(f'{save_dir}/data.npz', **data)
    save_videos_from_pil(kps_results, f'{save_dir}/dwpose.mp4', fps=fps)
    gc.collect()
    # if finetune_data:
    #     process_video(save_dir, video_name)

def online_process_video(video_path):
    ...
    pose_score = np.array(pose_score)
    body_score = pose_score[:, :18]
    face_score = pose_score[:, 24:92].reshape(-1, 1, 68)
    hands_score = pose_score[:, 92:].reshape(-1, 2, 21)
    
    weighted_heatmaps = []
    for i in tqdm(range(video_length)):
        ori = draw_pose(pose_pos[i], pose_id[i], face_pos[i], hands_pos[i], body_score[i], face_score[i], hands_score[i], h, w)
        ori = Image.fromarray(ori)
        weighted_heatmaps.append(ori)
        if i % 60 == 0:  # Adjust this value based on your system's memory capacity
            gc.collect()
    return weighted_heatmaps

def online_process_image(image_path):
    detector = DWposeDetector()
    detector = detector.to("cuda:0")
    ...
    result, score, pose = detector(image_pil, detect_resolution=512, image_resolution=512)
    return result
...
if __name__ == "__main__":
    ...
    todo_video = glob.glob('/projects/p32321/fating/dataset/PKU-DynamicaHuman/dynamichuman/*/*/crop_video_clip.mp4')
    allocated_fps = [todo_video[i] for i in range(len(todo_video)) if i % args.num_workers == args.job_idx]
    
    print("lens: ", len(todo_video))
    detector = DWposeDetector()
    detector = detector.to("cuda:0")
    for video_name in tqdm(allocated_fps):
        save_dir = os.path.join(os.path.dirname(video_name),'dwpose')
        os.makedirs(save_dir,exist_ok=True)
        if os.path.exists(f'{save_dir}/weighted_heatmap.mp4'):
            continue
        process_single_video(
            video_name, detector,  save_dir,
        )
        process_video(save_dir, video_name)  