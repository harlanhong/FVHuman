...
import pdb
import PIL
from models.mutual_self_attention import ReferenceAttentionControl,ReferenceAttentionControlv5
from utils.util import attention_map_to_image,all_attention_map_to_image

@dataclass
class Pose2ImagePipelineOutput(BaseOutput):
    images: Union[torch.Tensor, np.ndarray]
    attn_fea_img: Union[PIL.Image.Image]
...
class Pose2ImagePipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        num_inference_steps,
        guidance_scale,
        AttnControl=ReferenceAttentionControlv5,
        ...
        **kwargs,
        ...
        attn_fea = [self.attn_guider(pose_cond_tensor, pose_ref) for pose_ref in ref_pose_cond_tensor_list]
        # pdb.set_trace()
        attn_fea_img = all_attention_map_to_image(attn_fea,ref_image_list)
        # attn_fea_img = [attention_map_to_image(fea[0,0],ref_image) for fea,ref_image in zip(attn_fea,ref_image_list)]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.denoising_unet(
                    latent_model_input,
                    t,
                    encoder_hidden_states=torch.cat(image_prompt_embeds_list+[env_code],1),
                    attn_feat=attn_fea,
                    pose_feat=pose_fea,
                    return_dict=False,
                )[0]
        ...
        return Pose2ImagePipelineOutput(images=image,attn_fea_img=attn_fea_img)
