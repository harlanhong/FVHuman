...
class CrossPoseGuider(ModelMixin):
    def __init__(self, conditioning_channels = 3, conditioning_embedding_channels=320, use_ca=True):
        super(CrossPoseGuider, self).__init__()

        self.use_ca = use_ca

        self.conv_layers = nn.Sequential(
            nn.Conv2d(in_channels=conditioning_channels, out_channels=3, kernel_size=3, padding=1),
            nn.BatchNorm2d(3),
            nn.ReLU(),
        ...
        noise_latent_channels = conditioning_embedding_channels
...
#the channle number is the same as the unet
class CrossPoseGuiderv2(ModelMixin):
    def __init__(self, conditioning_channels = 3, conditioning_embedding_channels=320, use_ca=True):
        super(CrossPoseGuiderv2, self).__init__()
        ...
        if self.use_ca:
            self.cross_attn1 = Transformer2DModel(in_channels=noise_latent_channels*2)
            self.cross_attn2 = Transformer2DModel(in_channels=noise_latent_channels*4)
            self.cross_attn3 = Transformer2DModel(in_channels=noise_latent_channels*4)
    ...
    def _initialize_weights(self):
        # Initialize weights with He initialization and zero out the biases
        conv_blocks = [self.conv_layers, self.conv_layers_1, self.conv_layers_2, self.conv_layers_3]
        for block_item in conv_blocks:
            for m in block_item:
                if isinstance(m, nn.Conv2d):
                    n = m.kernel_size[0] * m.kernel_size[1] * m.in_channels
...
class SpatialAttn(ModelMixin):
    ...
    def _initialize_weights(self):
        # Initialize weights with He initialization and zero out the biases
        conv_blocks = [self.conv_layers, self.conv_layers_1]
        for block_item in conv_blocks:
            for m in block_item:
                if isinstance(m, nn.Conv2d):
                    n = m.kernel_size[0] * m.kernel_size[1] * m.in_channels
    ...
    def forward(self, x, ref_x):
        ...
        if self.use_ca:
            ...
            ref_x = self.conv_layers_1(ref_x)
            x = self.cross_attn1(x, ref_x)
        attn = rearrange(x, "(b f) c h w -> b c f h w", b=b)
...
#生成一个 c=1的attn map
class SpatialAttn2(ModelMixin):
    def __init__(self, conditioning_channels = 3, conditioning_embedding_channels=320, use_ca=True):
        super(SpatialAttn2, self).__init__()
        ...
        self.conv_layers_ref = nn.Sequential(
            nn.Conv2d(in_channels=conditioning_channels, out_channels=3, kernel_size=3, padding=1),
            nn.BatchNorm2d(3),
            nn.ReLU(),
            nn.Conv2d(in_channels=3, out_channels=16, kernel_size=4, stride=2, padding=1),
        ...
        # Final projection layer
        self.final_proj_ref = nn.Conv2d(in_channels=128, out_channels=noise_latent_channels, kernel_size=1)
        
 
        
        self.conv_layers_1_ref = nn.Sequential(
            nn.Conv2d(in_channels=noise_latent_channels, out_channels=noise_latent_channels, kernel_size=3, padding=1),
            nn.BatchNorm2d(noise_latent_channels),
            nn.ReLU(),
            nn.Conv2d(in_channels=noise_latent_channels, out_channels=noise_latent_channels*2, kernel_size=3, stride=2, padding=1),
        ...
        self.scale = nn.Parameter(torch.ones(1) * 2)
        self.scale_ref = nn.Parameter(torch.ones(1) * 2)
    # def _initialize_weights(self):
    #     # Initialize weights with Gaussian distribution and zero out the final layer
    #     for m in self.conv_layers:
    #         if isinstance(m, nn.Conv2d):
    ...
    def forward(self, x, ref_x):
        ...
        x = self.conv_layers_1(x)
        if self.use_ca:
            ref_x = self.conv_layers_ref(ref_x)
            ref_x = self.final_proj_ref(ref_x)
            ref_x = ref_x * self.scale_ref
            ref_x = self.conv_layers_1_ref(ref_x)
            x = self.cross_attn1(x, ref_x)
        attn = rearrange(x, "(b f) c h w -> b c f h w", b=b)
        attn = attn.mean(1,keepdim=True)
...
#生成一个 c=1的attn map ref对tat的attn
class SpatialAttn3(ModelMixin):
    def __init__(self, conditioning_channels = 3, conditioning_embedding_channels=320, use_ca=True):
        super(SpatialAttn3, self).__init__()
    ...
    def forward(self, x, ref_x):
        ...
        if self.use_ca:
            ...
            ref_x = ref_x * self.scale_ref
            ref_x = self.conv_layers_1_ref(ref_x)
            tgt_x = rearrange(x, "(b f) c h w -> f b c h w", b=b)
            rst = []
            for i in range(tgt_x.shape[0]):
                rst.append(self.cross_attn1(ref_x, tgt_x[i]))
            x = rearrange(torch.stack(rst), "f b c h w -> (b f) c h w", b=b)
        attn = rearrange(x, "(b f) c h w -> b c f h w", b=b)
        attn = attn.mean(1,keepdim=True)
