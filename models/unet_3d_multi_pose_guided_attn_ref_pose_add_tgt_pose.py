...
# pose guided add to the self-attention
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if tgt_fea is not None:
            sample = sample + tgt_fea[0]
        # down
        down_block_res_samples = (sample,)
        block_count = 0
        for downsample_block in self.down_blocks:
            # print(guidance_fea[block_count].shape)
            if (
                ...
                sample, res_samples = downsample_block(
                    ...
                    guidance_fea = guidance_fea[block_count],
                )
            else:
                sample, res_samples = downsample_block(
                    hidden_states=sample,
            ...
            # if tgt_fea is not None:
            #     sample = sample + tgt_fea[block_count]
            block_count += 1
